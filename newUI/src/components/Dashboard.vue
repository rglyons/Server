  ...
  <v-container fluid class="px-4 py-3">
    ...
    <v-row id="bottomBoxes">
      <v-col xs12 v-for="i in items" sm6 lg3 class="border" :key="newboxes[i].type">
        <bot-box @click.native.stop="chooseSensor(newboxes[i].type)" :chosen="chosenSensor==newboxes[i].type" :node="chosenNode" :apiKey="apiKey" :boxType="newboxes[i].type" :data="parseDataValue(newboxes[i])" :good="checkBoxStatus(newboxes[i])" :ideal="parseIdealRangeHtml(newboxes[i].ideal, newboxes[i].type)" :min="newboxes[i].ideal[0]" :max="newboxes[i].ideal[1]" class="topBox"></bot-box>
      </v-col>
    </v-row>
  </v-container>
</template>
